...
public:
    ...
    DynamicArray(const DynamicArray& other)
        ...
        cout << "A completely independent copy was made using the copy constructor.\n";
    }
    ...
    DynamicArray& operator=(const DynamicArray& other) {
        ...
        data = new int[capacity];
        for (int i=0; i<size_; i++) data[i] = other.data[i];
        cout << "Another independent copy created using the assignment operator.\n";
        return *this;
    }
...
void runDemo(Platform& p) {
    ...
    cout << "  Admin accounts active right now: "
         << AdminUser::getAdminCount() << "\n";
    cout << "  Total users on the platform: "
    ...
    cout << "r1: " << r1 << "\n";   // operator<<
    cout << "r2: " << r2 << "\n";
    cout << "Is r1 rated higher than r2?  " << (r1 > r2 ? "Yes" : "No") << "\n"; 
    cout << "Is r2 rated lower than r3?   " << (r2 < r3 ? "Yes" : "No") << "\n";  
    cout << "Do r1 and r3 have equal scores? " << (r1 == r3 ? "Yes" : "No") << "\n"; 
    Rating avg = r1 + r2;  // operator+
    cout << "Average of r1 and r2:  "; avg.displayStars();
    ...
    cout << "Via StreamingEngine pointer: "; sPtr->stream();
    cout << "Admin accounts currently active: " << AdminUser::getAdminCount() << "\n";
    admin.banUser(42);
    ...
    cout << "Same template class, two different types: WatchHistory<Movie> and WatchHistory<Documentary>.\n";
    ...
    cout << "\n========== Dynamic Array with Copy Constructor ==========\n";
    DynamicArray a1(4);
    a1.push(10); a1.push(20); a1.push(30);
    a1.display();
    DynamicArray a2 = a1;  
    a2.push(40);
    cout << "a1 after modifying a2 (should be unchanged): "; a1.display();
    cout << "a2 after push (independent deep copy):        "; a2.display();
    DynamicArray a3(2); a3 = a1;
    cout << "a3 after assignment from a1:                  "; a3.display();
    cout << "Accessing a1[1] using operator[]:  " << a1[1] << "\n";
    try { cout << a1[99]; }
    catch (const out_of_range& e) { cerr << "Caught an exception: " << e.what() << "\n"; }

    cout << "\n========== Exception Handling - 7 Tests ==========\n";
    cout << "\nTest 1: What happens if you try to log in with blank fields?\n";
    safeLogin(p, "", "");
    cout << "\nTest 2: What happens if the email has no @ symbol?\n";
    safeRegister(p, "User", "notemail", "pass1234");
    cout << "\nTest 3: What if the password is too short?\n";
    safeRegister(p, "User", "u@x.com", "ab");
    cout << "\nTest 4: What if someone passes a negative movie ID?\n";
    safeWatch(p, -1);
    cout << "\nTest 5: Can you watch something without logging in first?\n";
    safeWatch(p, 1);
    cout << "\nTest 6: Let's do a full run - register, log in, and watch a movie.\n";
    safeRegister(p, "Demo User", "demo@x.com", "pass1234");
    safeLogin(p, "demo@x.com", "pass1234");
    if (p.isLoggedIn()) { safeWatch(p, 1); p.logout(); }
    cout << "\nTest 7: Out-of-range array access was demonstrated above.\n";

    cout << "\n========== Stack (LIFO) using DynamicArray ==========\n";
    Stack st("ViewHistory");
    st.push(101); st.push(202); st.push(303);
    st.display();
    cout << "Top of the stack (peek, no removal): " << st.peek() << "\n";
    st.pop(); st.pop(); st.display();
    try { Stack empty("Empty"); empty.pop(); }
    catch (const underflow_error& e) { cerr << "Caught an exception: " << e.what() << "\n"; }

    cout << "\n========== Queue (FIFO) using DynamicArray ==========\n";
    ...
    cout << "\n========== STL Containers (map, queue, stack) ==========\n";
    ContentCatalog cat;
    cat.addContent(1,"Inception");
    cat.addContent(2,"Dark Knight");
    cat.addContent(10,"Planet Earth II");
    cat.showAll();
    cout << "Lookup ID 2  -> " << cat.findByID(2) << "\n";
    cout << "Lookup ID 99 -> " << cat.findByID(99) << "\n";
...
int main() {
    ...
    while (true) {
        ...
        else if (choice == 0) { cout << "Closing CineStream. Goodbye!\n"; return 0; }
        else cout << "That's not a valid option. Please try again.\n";
    }
}
